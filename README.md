...
```bash
npm install @stdlib/stats-base-dsemyc
```
...
var v = dsemyc( 4, 1, x1, 2 );
// returns 1.25
```

#### dsemyc.ndarray( N, correction, x, strideX, offsetX )
...
#### stdlib_strided_dsemyc( N, correction, \*X, strideX )

Computes the [standard error of the mean][standard-error] of a double-precision floating-point strided array using a one-pass algorithm proposed by Youngs and Cramer.

```c
const double x[] = { 1.0, -2.0, 2.0 };

double v = stdlib_strided_dsemyc( 3, 1.0, x, 1 );
// returns ~1.20185
```
...
```c
double stdlib_strided_dsemyc( const CBLAS_INT N, const double correction, const double *X, const CBLAS_INT strideX );
```

#### stdlib_strided_dsemyc_ndarray( N, correction, \*X, strideX, offsetX )
...
double v = stdlib_strided_dsemyc_ndarray( 3, 1.0, x, 1, 0 );
// returns ~1.20185
```
...
```c
double stdlib_strided_dsemyc_ndarray( const CBLAS_INT N, const double correction, const double *X, const CBLAS_INT strideX, const CBLAS_INT offsetX );
```
...
[test-image]: https://github.com/stdlib-js/stats-base-dsemyc/actions/workflows/test.yml/badge.svg?branch=main
[test-url]: https://github.com/stdlib-js/stats-base-dsemyc/actions/workflows/test.yml?query=branch:main
